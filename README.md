...
1. 用户空间代码如何陷入内核态
1. 执行完系统调用后，如何返回到用户空间中
1. 如何在用户态和内核态的切换中保存状态，以达到让程序无感切换
1. 各个关键寄存器的转变节点在哪里（pc，ra，epc，sp，sterve）
...
1. sigreturn 有啥用。（**sigreturn会在handler结束后调用，是用于恢复执行handler之前寄存器状态的函数，特别是 pc 寄存器**）
1. 为了不干扰后续的trap执行流，是不是要把当前进程用户状态的数据保存两份。（**是的，要新增一个类似trapframe的容器页，用来保存执行handler函数前的寄存器状态，不然执行handler会修改掉部分寄存器状态**）
...
1. （如果trap的原因不是设备中断）将sstatus中的SIE位清0，关闭设备中断。
1. 将pc的值复制到sepc中。
1. 将发生trap的当前模式（用户模式或监管者模式）写入sstatus中的SPP位。
1. 设置scause的内容，反映trap的起因。
1. 设置模式为监管者模式（注意，此时只是将权限提升至内核级别，但是页表并没有发生切换）。
1. 将stvec的值复制到pc中（每个进程的在初始化的时候都会将寄存器stvec指向trampoline.s的uservec）。
1. 从新的pc值开始执行。
...
1. 将当前用户空间下的寄存器保存至trapframe页中（包括原本a0寄存器，具体参考uservec代码）
1. 恢复一些内核信息（sp，cpuid，usertrap，内核stap）
1. 将页表切换为内核页表，并把原本用户空间的页表保存到寄存器t1中
1. 跳转到usertrap的地址开始执行函数（**此刻完全由用户态陷入到内核态，接下来的执行流程由内核掌控**）
...
1. 重新保存内核信息到trapframe中（cpuid，内核栈，页表，kerneltrap）
1. 重置sstatus，打开中断（**这里的中断是外部触发的中断，不是在内核运行时可能出现的中断**）。
1. 重置sepc指针，让它指向之前用户空间下的地址
1. 执行userret函数
...
1. 切换页表，转化为用户进程页表
1. 将之前保存在trapframe中的寄存器恢复
1. 调用sret 将sepc和pc寄存器互换，至此程序执行流恢复到用户空间
...
https://zhuanlan.zhihu.com/p/318448376
