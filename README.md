...
1. 直接引用数据块（12个）。
1. 间接引用数据块（用一个直接引用数据块做间接引用数据块 256个）
1. 每个数据块的大小是1024b。
...
1. 特点：按需分配，不是在初始化时就将268个数据块全部分配，而是在需要的时候在去分配。
...
1. 文件操作的大致流程。
1. 流程中的关键节点（如何转化逻辑块号到物理块号，详情见large file）。
1. 文件的目录结构是怎么实现的。
1. 怎么按照目录结构递进搜索。
...
1. 找到新文件路径上一层节点得inode地址，dp = nameiparent(new, name)。
1. 然后将旧文件得inode编号复制给新文件，这样就建立了新旧文件得映射关系。
